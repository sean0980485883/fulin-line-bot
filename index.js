...
const PRODUCTS = {
  ...
  "沙門": { label: "沙門（安全紗門）", description: "菱格鐵網安全紗門，通風防盜\n📩 如需報價請洽詢", images: [img("沙門_01.png"), img("沙門_01.png"), img("沙門_01.png")] },
  "DM": { label: "富林組合屋 產品DM", description: "富林組合屋\n雲林縣二崙鄉楊賢路143號\n\n單顆入門基礎款（1門2窗）\n3mx6m  NT$98,000\n4mx6m  NT$118,000\n4mx8m  NT$156,000\n\n衛浴加購：大衛浴 NT$55,000 / 小衛浴 NT$40,000\n\n廖先生 0929-010-882 / Line：@aa168", images: [img("DM_01.jpg"), img("DM_02.jpg"), img("DM_03.jpg")] },
};
  ...
  for (const event of events) {
    ...
    try {
      if (["@目錄","@型錄","@產品","@品項","@所有產品"].includes(text)) {
        await replyMessages(replyToken, [{ type: "text", text: "📋 富林組合屋 產品目錄\n（輸入 @品名 查看照片）\n\n💰 輸入「@DM」查看價格\n\n🏠 框架搭配：\n@白框木紋 @白框白板 @白框灰黑\n@黑框木紋 @黑框白板 @黑框灰黑\n\n🚿 衛浴系列：\n@大衛浴 @小衛浴 @水泥廁所\n@室外衛浴 @貼磁衛浴 @日式衛浴\n\n🏠 組合屋系列：\n@二樓 @三樓 @展翼屋 @20呎展翼屋 @折疊屋 @宿舍\n\n🍳 配備系列：\n@廚具 @SPC地板 @三合一門 @標準窗 @沙門" }]);
        continue;
      }
      if (["@匯款","@帳號","@轉帳","@付款"].includes(text)) {
        await replyMessages(replyToken, [{ type: "text", text: "💳 富林組合屋 匯款資訊\n\n銀行：華南銀行 西螺分行\n總行代號：008\n帳號：542-20-020369-5\n戶名：廖國賓\n\n匯款後請傳截圖確認！" }]);
        continue;
      }
      if (["@聯絡","@電話","@contact"].includes(text)) {
        await replyMessages(replyToken, [{ type: "text", text: "📞 富林組合屋\n廖先生 0929-010-882\nLine：@aa168\n地址：雲林縣二崙鄉楊賢路143號" }]);
        continue;
      }
...
app.get("/", (req, res) => { res.json({ status: "ok", service: "fulin-line-bot", products: Object.keys(PRODUCTS).length, base_url: BASE_URL, token_ok: !!LINE_CHANNEL_ACCESS_TOKEN }); });
app.listen(PORT, () => { console.log("LINE bot running on port", PORT); console.log("BASE_URL:", BASE_URL); });const express = require("express");
const crypto = require("crypto");
const axios = require("axios");
const path = require("path");
